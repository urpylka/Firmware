...
void BlockLocalPositionEstimator::gpsCorrect()
{
	...
	_pub_gain.get().gps_pos_gain[0] = K(X_x, Y_gps_x);
	_pub_gain.get().gps_pos_gain[1] = K(X_y, Y_gps_y);
	_pub_gain.get().gps_pos_gain[2] = K(X_y, Y_gps_z);
	_pub_gain.get().gps_vel_gain[0] = K(X_vx, Y_gps_vx);
	_pub_gain.get().gps_vel_gain[1] = K(X_vy, Y_gps_vy);
	_pub_gain.get().gps_vel_gain[2] = K(X_vz, Y_gps_vz);
}
